                        ...
                        <form action="{{ route('admin.product.update',$product->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            @method('PUT')
								...
								<div class="row container">
                                    ...
                                    <div class="form-group p-4 col-6" style="background-color:#FBF4FA;">
                                        <div class="form-group col-12">
                                            <label>{{__('admin.Product video Thumbnail One')}} (Optional)</label><hr>
                                            <div>
                                                <img id="preview-video" class="admin-img" src="{{ asset($product->video_thumb) }}" alt="">
                                            </div>

                                            <div class="form-group col-12">
                                            <label><span class="text-danger"></span></label>
                                            <input type="file" class="form-control-file"  name="video_thumb" onchange="previewThumnailVideo(event)">
                                            </div>

                                        </div><hr>

                                        <label>{{__('admin.Product Video Link One')}} <br>
                                        Enter Youtube Embed URL Eg: <br><span class="text-danger">https://www.youtube.com/embed/mQwhTphlb98</span>
                                        </label>
                                        <input  type="text" id="product-video" class="form-control bg-white"  name="product_video" value="{{ $product->product_video }}">
                                        <span class="text-danger">(To hide the video remove the video URL from above text box)</span>
                                    </div>

                                    <div class="form-group p-4 col-6" style="background-color:aliceblue;">
                                        <div class="form-group col-12">
                                            <label>{{__('admin.Product video Thumbnail Second')}} (Optional)</label><hr>
                                            <div>
                                                <img id="preview-video-sec" class="admin-img" src="{{ asset($product->video_thumb_sec) }}" alt="">
                                            </div>

                                            <div class="form-group col-12">
                                            <label><span class="text-danger"></span></label>
                                            <input type="file" class="form-control-file"  name="video_thumb_sec" onchange="previewThumnailVideoSec(event)">
                                            </div>

                                        </div><hr>

                                        <label>{{__('admin.Product Video Link Second')}} 
                                        <br>
                                        Enter Youtube Embed URL Eg: <br><span class="text-danger">https://www.youtube.com/embed/mQwhTphlb98</span>
                                        </label>
                                        <input type="text" id="product-video-sec" class="form-control bg-white"  name="product_video_sec" value="{{ $product->product_video_sec }}">
                                        <span class="text-danger">(To hide the video remove the video URL from above text box)</span>
                                    </div>
                                </div>
...
<script>
        ...
        $(document).ready(function () {
            ...
            $(".removeExistSpecificationRow").on("click",function(){
                ...
                $.ajax({
                    ...
                    url:"{{url('/removed-product-exist-specification/')}}"+"/"+specificationId,
                    success:function(response){
    ...
    function previewThumnailVideoSec(event) {
        var reader = new FileReader();
        reader.onload = function(){
            var output = document.getElementById('preview-video-sec');
            output.src = reader.result;
        }
        reader.readAsDataURL(event.target.files[0]);
    };
