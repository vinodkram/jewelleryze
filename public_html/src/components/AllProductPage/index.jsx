...
export default function AllProductPage({ response, sellerInfo }) {
  ...
  const [subCategoryExistInRoute, setSubCategoryExistInRoute] = useState(false);
  const [childCategoryExistInRoute, setChildCategoryExistInRoute] = useState(false);
  ...
  useEffect(() => {
    if (response.data && response.data.products.data.length === 0) {
      if (router.query.search && router.query.sub_category) {
        setSubCategoryExistInRoute(true);
      }
    }
  }, [response]);
  useEffect(() => {
    if (response.data && response.data.products.data.length === 0) {
      if (router.query.search && router.query.child_category) {
        setChildCategoryExistInRoute(true);
      }
    }
  }, [response]);
  useEffect(() => {
  ...
  const [resTotalProducts, setTotalProducts] = useState(null);
  ...
  useEffect(() => {
    ...
    //           item.active_sub_categories.map((variant_item) => {
    ...
    const min = response.data &&
        ...
        Math.min(
            ...response.data.products.data.map((item) => parseInt(item.offer_price))
        );
      ...
      if (check) {
        ...
        axios
          .get(
            ...
            }min_price=${volume[0]}&search=${router.query.search}&max_price=${volume[1]}${
              sellerInfo ? `&shop_name=${sellerInfo.seller.slug}` : ""
            }${
              router.query.category ? `&category=${router.query.category}` : ""
            }${
              router.query.sub_category ? `&sub_category=${router.query.sub_category}` : ""
            }${
              router.query.child_category ? `&child_category=${router.query.child_category}` : ""
            }`
          )
          .then((res) => {
            ...
            const resTotalProducts =  response.data.products.total;
            res.data && res.data.products.data.length > 0
              ...
              <div className="flex-1">
                {response.data && response.data.products.data.length > 0 ? (
                  <div className="w-full">
                    ...
                    >
          {resTotalProducts && resTotalProducts > 0 ? (
                      <div>
                        <p className="font-400 text-[14px] text-qgray capitalize">
                          ...
                          1–
                          {resTotalProducts < response.data.products.per_page ? resTotalProducts : response.data.products.data.length}{" "}
            ...
            <div>
                      ...
                      <div className="flex space-x-3 items-center">
                      <span className="font-bold  lg:hidden text-qblack text-[13px]">
                          {langCntnt && langCntnt.FilterByProductCategories} :
                        </span>
                      <button
            ...
            <div>
              ...
              {response.data && response.data.shopPageSidebarBanner && (
                  <div
                    ...
                    className="w-full py-[35px] pl-[40px] group xl:block h-[295px] relative rounded"
                  >
                        ...
                        >
                          <a rel="noopener noreferrer">
                            <div className="cursor-pointer w-full relative  ">
                              <div className="inline-flex rounded-full space-x-1.5 items-center relative z-20 bg-qyellow hover:bg-amber-300">
                                <span className="text-sm text-qblack font-medium leading-[30px] pl-2">
                              ...
                              {/* <div className="w-[82px] transition-all duration-300 ease-in-out group-hover:h-4 h-[2px]  absolute left-0 bottom-0 z-10"></div> */}
                            </div>
                          </a>
                        </Link>
                      </div>
