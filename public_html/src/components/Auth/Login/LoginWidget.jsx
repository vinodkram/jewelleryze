...
function LoginWidget({ redirect = true, loginActionPopup, notVerifyHandler }) {
  ...
  useEffect(() => {
    if (!defaultProfileImg) {
      if (websiteSetup) {
        //console.log(websiteSetup.payload.setting.phone_number_required);
  ...
  //console.log(phone_number_required);
  useEffect(() => {
      ...
      <div className="input-area">
        ...
        {phone_number_required == "1" && (
          ...
          <div className="input-item mb-5">
            <InputCom
              placeholder={langCntnt && langCntnt.Email_Phone}
              label={langCntnt && langCntnt.Email_Phone + "*"}
              name="email"
              type="email"
              inputClasses="h-[50px]"
              value={email}
            ...
            <span className="text-red-500 text-xs  ml-1">
             {langCntnt && langCntnt.Login_instruction}
            </span>
          </div>
        )}
