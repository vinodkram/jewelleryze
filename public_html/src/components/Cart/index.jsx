...
export default function Cart({ className }) {
  ...
  useEffect(() => {
    if (getCarts && getCarts.length > 0) {
        ...
        getCarts.map((v) => {
          if (v.product.offer_price) {
            if (v.variants && v.variants.length > 0) {
              ...
              const v_price = ((sumCal + parseFloat(v.product.offer_price)) * v.qty);
              return checkProductExistsInFlashSale(v.product_id, v_price);
            } else {
              const wo_v_price = parseFloat(v.product.offer_price * v.qty);
            ...
            if (v.variants && v.variants.length > 0) {
              ...
              const sumCal = prices.reduce((p, c) => p + c);
              const v_price = ((sumCal + parseFloat(v.product.price)) * v.qty);
              return checkProductExistsInFlashSale(v.product_id, v_price);
            } else {
              const wo_v_price = parseFloat(v.product.price * v.qty);
              return checkProductExistsInFlashSale(v.product_id, wo_v_price);
            }
          }
      ...
      // code to show payment cancelled status start
      const query = new URLSearchParams(window.location.search);
      const myParam = query.get("pay-status");
      if(myParam === 'false'){
         toast.error(langCntnt && langCntnt.Payment_Failed, { autoClose: 1000,toastId: 'success1', });
      }
      // code to show payment cancelled status end
    ...
    if (item) {
      if (item.product.offer_price) {
        if (item.variants && item.variants.length > 0) {
          ...
          return parseFloat(((item.product.offer_price) + sumVarient) * item.qty);
        } else {
        ...
        if (item.variants && item.variants.length > 0) {
          ...
          const sumVarient = prices.reduce((p, c) => p + c);
           return parseFloat(((item.product.price) + sumVarient) * item.qty);
        } else {
          return parseFloat(item.product.price * item.qty);
        }
      }
    }
  };
      ...
      >
        {getCarts && getCarts.length > 0 ? (
          ...
          <div className="w-full">
            ...
            <div className="w-full px-4 abc-class mt-[20px]">
              <div className="h-[1px] bg-[#F0F1F3]"></div>
            </div>
