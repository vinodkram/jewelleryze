...
function CustomPageCom({ slug }) {
  ...
  useEffect(() => {
    if (websiteSetup) {
	  ...
	  const selectedPageSeo = websiteSetup.payload.seo_setting.find((obj) => { 
        return obj.page_slug === slug; 
      });
      if (checkPageIsExist) {
	  ...
	  if (selectedPageSeo) {
        setPageSeo(selectedPageSeo);
      }
    }
  }, [pageData, pageSeo, slug, websiteSetup]);
  return (
    <>
      {pageData && 
      <PageHead 
      title={pageSeo.seo_title}
      metaDes={pageSeo.seo_description}
      />}
