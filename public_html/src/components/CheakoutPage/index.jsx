...
function CheakoutPage() {
  ...
  const [addresses, setAddresses] = useState(null);
    //by abhishek start
  const [productIs, setBuyNowProduct] = useState(null);
  // by abhishek end
  ...
  const [selectPayment, setPaymentMethod] = useState("cashOnDelivery");
  ...
  //bank status
  const [cashOnDeliveryStat, setCashOnDeliveryStatus] = useState(null);
  const [stripeStatus, setStripeStatus] = useState(null);
  const [rezorPayStatue, setRezorPay] = useState(null);
  const [ccavenueStatue, setCcavenue] = useState(null);
  // console.log('setRezorPay='+setRezorPay);
  // console.log('rezorPayStatue='+rezorPayStatue);
  // console.log('setCcavenue='+setCcavenue);
  // console.log('ccavenueStatue='+ccavenueStatue);
  ...
  const removeCoupon = () => {
    if (auth()) {
      ...
      toast.error(langCntnt && langCntnt.Coupon_Removed_Successfully);
    ...
    axios
      ...
      .then((res) => {
        ...
        setCcavenue(
          !!(
            res.data &&
            res.data.ccavenuePaymentInfo &&
            res.data.ccavenuePaymentInfo.status == 1
          )
        );
  ...
  useEffect(() => {
    ...
    //custom code for buy now start
    const search = window.location.search;
    const query = new URLSearchParams(search);
    const productIs = query.get('product')
    if (productIs !== undefined && productIs !== null && cart !== null) {
      setCarts(cart && cart.cartProducts);
      const filtered = cart.cartProducts.filter((car) => {
        return car.id === JSON.parse(productIs);
      });

      setBuyNowProduct(productIs);
      ...
      setCarts(cart && cart.cartProducts);
      //console.log('normal checkout:', productIs);
    };
    //custom code for buy now end
  ...
  useEffect(() => {
    if (shippingCharge) {
      setGstAmount(parseFloat(((totalPrice + parseFloat(shippingCharge)) * tax_rate) / 100).toFixed(2));
      setMainTotalPrice((totalPrice + parseFloat(gstAmount) + parseFloat(shippingCharge)).toFixed(2));
      
    } else {
      setGstAmount(parseFloat((totalPrice * tax_rate) / 100).toFixed(2));
      setMainTotalPrice((totalPrice + parseFloat(gstAmount)).toFixed(2));
  ...
  const placeOrderHandler = async () => {
    if (auth()) {
        if (selectedBilling && selectedShipping) {
          //console.log("Shipping Billing Exists");
        } else {
          toast.error(langCntnt && langCntnt.No_Shipping_Address_Found);
        }

        if (selectedBilling && selectedShipping) {
        //console.log("in selected billing ");
        if (selectedRule) {
          if (selectPayment) {
            //alert('selectPayment='+selectPayment);
            if (selectPayment && selectPayment === "cashOnDelivery") {
              await apiRequest
                .cashOnDelivery(
                  {
                    buy_now_productid: parseInt(productIs),
                ...
                .then((res) => {
                  if (res.data) {
                    ...
                    toast.success(res.data.message, { position: toast.POSITION.TOP_LEFT, className: 'toast-message-confirm-order', autoClose: 5000 });
            ...
            } else if (selectPayment && selectPayment === "ccavenue") {
              const url = `${
                process.env.NEXT_PUBLIC_BASE_URL
              }user/checkout/ccavenue-order?token=${
                auth().access_token
              }&shipping_method_id=${parseInt(
                selectedRule
              )}&shipping_address_id=${selectedShipping}&buy_now_productid=${productIs}&coupon=${
                couponCode && couponCode.code
              }&billing_address_id=${selectedBilling}`;
              //console.log('url for checkout='+url);
              await axios
                .get(url)
                .then((res) => {
                  const order_id = res.data && res.data.order_id;
                  ...
                  if (res.data) {
                    const provideUrl = `${
                      process.env.NEXT_PUBLIC_BASE_URL
                    }user/checkout/ccavenue-web-view?token=${
                      auth().access_token
                    }&shipping_address_id=${selectedShipping}&coupon=${
                      couponCode && couponCode.code
                    }&billing_address_id=${selectedBilling}&shipping_method_id=${parseInt(
                      ...
                      typeof window !== "undefined" && window.location.origin
                        ? window.location.origin + "/cart?pay-status=false"
                        : ""
                    }&request_from=react_web&amount=${amount}&order_id=${order_id}`;
                    router.push(provideUrl);
                    localStorage.removeItem("coupon_set_date");
            ...
            //console.log("out selected billing 1");
            toast.error( langCntnt && langCntnt.Please_Select_Your_Payment_Method );
          }
        } else {
          //console.log("out selected billing 2");
                  ...
                  <div className="addresses-widget w-full">
                    <div className="sm:flex justify-between items-center w-full mb-5">
                      ...
                      <button
                        ...
                        type="button"
                        className="w-[200px] h-[40px] mt-2 sm:mt-0 border rounded-full text-qblack hover:bg-qblack hover:text-white bg-qyellow transition-all duration-300 ease-in-out"
                      >
                      ...
                      <form>
                        ...
                        <div className=" mb-6">
                          <div className="w-full">
                            <InputCom
                              value={city_name}
                              inputHandler={(e) => setCityName(e.target.value)}
                              label={langCntnt && langCntnt.City + "*"}
                              placeholder={
                                langCntnt && langCntnt.City
                              }
                              inputClasses="w-full h-[50px]"
                              error={
                                !!(errors && Object.hasOwn(errors, "city_name"))
                              }
                            />
                            {errors && Object.hasOwn(errors, "city_name") ? (
                              <span className="text-sm mt-1 text-qred">
                        ...
                        <div className=" mb-6">
                          <div className="w-full">
                            <InputCom
                              ...
                              label={langCntnt && langCntnt.Pincode + "*"}
                              placeholder={
                                langCntnt && langCntnt.your_pincode_here
                              }
                              inputClasses="w-full h-[50px]"
                              error={
                ...
                <div className="mb-10">
                  ...
                  { couponCode && (
                    <button
                      ...
                      type="button"
                      className="w-[190px] mt-3 h-[40px]"
                    >
                      <div className=" rounded">
                        <span className="text-sm text-qred font-bold">
                          x {langCntnt && langCntnt.Remove_Coupon}
                        </span>
                      </div>
                    </button>
                  )}
                ...
                <div className="w-full px-10 py-[30px] rounded border border-qpurplelow">
                            ...
                            <div className="flex justify-between items-center">
                              <div>
                                <h4
                                  ...
                                  className="text-[15px] text-qblack line-clamp-1"
                                >
                                  {wordCount(`${item.product.name}`)}
                                  <span className="text-[13px] text-black font-bold ml-2 mt-2">
                                    X {parseInt(item.qty)}
                                  </span>
                                </h4>
				  ...
				  {tax_rate > 0 && (
                    <div className="mt-[10px] flex justify-between">
                      ...
                      >
                        {currency_icon
                          ? currency_icon + parseFloat(gstAmount).toFixed(2)
                      ...
                      <div className="flex flex-col  space-y-3">
                        ...
                        {ccavenueStatue && (
                          <div
                            ...
                            className={`payment-item ccavenue-btn text-center bg-[#F8F8F8] relative w-full h-[50px] font-bold text-sm text-white text-qpurple  rounded flex justify-center items-center px-3 uppercase rounded cursor-pointer ${
                              selectPayment === "ccavenue"
                                ? "border-2 border-qpurple"
                                : "border border-qpurplelow"
                            }`}
                          >
                            <div className="w-full flex justify-center">
                              <span>
                                ...
                                width="300" height="90" viewBox="0 0 550.000000 550.000000">

                                <g transform="translate(0.000000,550.000000) scale(0.100000,-0.100000)"
                                fill="#000000" stroke="none">
                                <path d="M649 3091 c-111 -30 -184 -92 -231 -197 -17 -39 -22 -68 -22 -139 -1
                                -114 20 -177 80 -244 55 -61 125 -96 224 -112 64 -10 86 -9 150 5 100 23 101
                                23 94 78 l-6 45 -60 -18 c-77 -24 -134 -24 -201 1 -57 21 -104 64 -132 120
                                -22 43 -26 168 -6 223 43 124 190 186 335 142 27 -8 53 -15 58 -15 5 0 8 21 6
                                47 l-3 47 -70 18 c-87 22 -132 22 -216 -1z"/>
                                <path d="M1303 3095 c-110 -30 -192 -96 -235 -190 -19 -42 -23 -67 -23 -160 0
                                -100 2 -115 28 -167 48 -97 136 -157 267 -178 67 -11 84 -11 160 6 l85 20 3
                                34 c4 58 -6 64 -75 46 -76 -20 -116 -20 -179 -1 -57 17 -124 79 -149 137 -17
                                42 -20 146 -5 197 38 133 182 201 334 157 32 -10 60 -16 61 -14 2 2 5 23 7 48
                                l3 44 -70 17 c-86 21 -144 22 -212 4z"/>
                                <path d="M2075 3088 c-2 -7 -34 -94 -70 -192 -36 -99 -65 -181 -65 -183 0 -2
                                -66 -3 -147 -3 -80 0 -144 -2 -142 -4 3 -3 58 -17 124 -31 66 -15 124 -31 128
                                -35 5 -4 -9 -58 -32 -120 -23 -61 -41 -114 -41 -116 0 -3 19 -4 42 -2 l42 3
                                35 105 c20 58 37 107 39 109 1 3 51 -7 111 -21 l110 -25 -25 65 c-13 35 -24
                                66 -24 68 0 2 -31 4 -69 4 l-70 0 47 140 c27 76 50 142 54 145 3 3 17 -29 32
                                -72 15 -43 61 -172 102 -288 42 -115 77 -216 79 -222 4 -11 85 -20 85 -9 0 3
                                -151 412 -222 601 l-33 90 -43 3 c-28 2 -44 -1 -47 -10z"/>
                                <path d="M4956 3001 c-36 -24 -55 -57 -56 -94 -1 -95 126 -152 187 -84 53 58
                                56 103 10 153 -41 45 -97 55 -141 25z m128 -32 c36 -43 35 -91 -3 -130 -64
                                -64 -161 -23 -161 68 0 59 34 93 96 93 35 0 47 -5 68 -31z"/>
                                <path d="M4971 2903 c1 -32 4 -46 6 -30 5 34 17 34 47 2 29 -31 43 -33 22 -3
                                -14 19 -14 24 -1 38 25 28 8 50 -36 50 l-39 0 1 -57z m64 27 c0 -9 -11 -16
                                -27 -18 -23 -3 -28 1 -28 18 0 17 5 21 28 18 16 -2 27 -9 27 -18z"/>
                                <path d="M3026 2906 c-57 -21 -87 -46 -120 -100 -25 -40 -30 -60 -34 -132 -7
                                -133 39 -224 133 -260 57 -21 164 -16 220 11 33 17 41 26 43 54 2 18 0 31 -5
                                29 -105 -53 -168 -60 -232 -26 -47 24 -74 64 -85 126 l-5 32 181 0 181 0 -5
                                64 c-7 83 -39 150 -88 182 -48 32 -129 40 -184 20z m134 -66 c30 -16 60 -70
                                60 -107 l0 -33 -135 0 c-131 0 -135 1 -135 21 0 37 42 98 81 117 39 20 95 20
                                129 2z"/>
                                <path d="M3576 2906 c-21 -8 -50 -26 -65 -40 -36 -34 -41 -33 -41 9 0 33 -2
                                35 -35 35 l-35 0 0 -250 0 -250 35 0 35 0 0 158 c0 144 2 162 23 202 29 58 65
                                83 119 83 104 0 118 -33 118 -268 l0 -175 41 0 41 0 -4 193 c-3 212 -12 247
                                -69 288 -37 26 -113 33 -163 15z"/>
                                <path d="M4595 2905 c-54 -19 -118 -91 -136 -152 -19 -64 -16 -161 5 -217 20
                                -53 78 -109 131 -126 49 -16 159 -8 208 16 32 15 37 22 37 51 l0 33 -42 -22
                                c-62 -32 -161 -32 -206 0 -37 27 -57 61 -67 115 l-7 37 178 2 179 3 -2 55 c-7
                                165 -131 257 -278 205z m183 -115 c41 -81 36 -85 -112 -88 -71 -1 -132 1 -136
                                5 -14 15 11 73 45 107 27 27 44 36 77 38 67 5 100 -11 126 -62z"/>
                                <path d="M2395 2873 c8 -21 50 -132 94 -248 l79 -210 39 -3 38 -3 95 242 c52
                                132 97 245 98 250 2 5 -14 9 -35 9 -36 0 -41 -3 -50 -32 -52 -157 -143 -390
                                -147 -378 -3 8 -36 103 -74 210 l-68 195 -42 3 -42 3 15 -38z"/>
                                <path d="M3932 2713 c3 -216 9 -241 69 -285 58 -44 160 -35 219 19 41 36 56
                                32 48 -13 -2 -15 4 -20 35 -22 l37 -3 -2 248 -3 248 -32 0 -32 0 -4 -165 c-3
                                -144 -6 -170 -25 -206 -38 -74 -141 -97 -197 -44 -36 34 -45 86 -45 259 l0
                                161 -36 0 -36 0 4 -197z"/>
                                </g>
                                </svg>
                              </span>
                            </div>
