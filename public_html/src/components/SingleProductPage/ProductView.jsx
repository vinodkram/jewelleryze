...
export default function ProductView({
  ...
  const [productsImgShow, setproductsImgShow] = useState(true);
  const [productsImgShow2, setproductsImgShow2] = useState(false);
  ...
  const changeImgHandler = (current) => {
    setproductsImgShow(current => true);
    setproductsImgShow2(current => false);
    setSrc(current);
  };

  const changeImgHandlerVideo = (link) => {
    setproductsImgShow2(current => false);
    setproductsImgShow(current => false);
	setSrc(link);
  };

  const changeImgHandlerVideoSecond = (link) => {
    setproductsImgShow2(current => true);
    setproductsImgShow(current => false);
	setSrc(link);
  }; 
  ...
  //custom code start for buy now button
  const buyNow = (id) => {
    const data = {
      id: product.id,
      token: auth() && auth().access_token,
    ...
    if (auth()) {
      if (varients) {
        ...
        apiRequest
          ...
          .then((res) =>{
            dispatch(fetchCart());
            router.push(`/checkout?product=${res.data.cartID}`);
          })
          .catch((err) => console.log(err));
      } else {
        const uri = `token=${data.token}&product_id=${data.id}&quantity=${data.quantity}`;
      ...
      >
        ...
        >
          <div className="w-full">
            ...
            {productsImgShow && !productsImgShow2 && (
            <div className="w-full md:h-[570px] h-full border border-qpurplelow flex justify-center items-center overflow-hidden relative mb-3">
              {/* <Image
                layout="fill"
                objectFit="scale-down"
                src={`${process.env.NEXT_PUBLIC_BASE_URL + src}`}
                alt=""
                className="object-contain  transform scale-75 hover:scale-150 duration-500"
              /> */}
              <InnerImageZoom 
                layout="fill"
                objectFit="cover"
                src={`${process.env.NEXT_PUBLIC_BASE_URL + src}`}
                alt="image"
                className="flex items-center justify-center main-img-w main-img-h" />
              
            </div>
            )}

            {!productsImgShow && !productsImgShow2 && (
            <div className="w-full mobile-height-sm md:h-[570px] h-full border border-qpurplelow flex justify-center items-center overflow-hidden relative mb-3">
              
			  <iframe
              width="100%" height="100%"
              src={product.product_video}
              title="FestivJwellery" frameBorder="0"
              allowfullscreen>
              </iframe>
            ...
            <div className="flex gap-2 flex-wrap">
              <div
                onClick={() => changeImgHandler(product.thumb_image)}
                 className="w-[110px] h-[110px] flex justify-center items-center overflow-hidden relative transform scale-100"
              >
                <Image
                  layout="fill"
                  objectFit="cover"
                  ...
                  className={`bg-contain bg-center ${
                    src !== product.thumb_image ? "" : "active-image"
                  } `}
                ...
                productsImg.map((img, i) => (
                  ...
                  >
                    <Image
                      ...
                      alt="image"
                      placeholder="blur"
                      blurDataURL="/assets/images/spinner.gif"
                      className={`bgloading bg-contain bg-center ${
                        src !== img.image ? "" : "active-image"
            ...
            {product.product_video && product.video_thumb && (
              <div
                onClick={() => changeImgHandlerVideo(product.product_video)}
                 className="w-[110px] h-[110px] product-video-thumb flex justify-center items-center overflow-hidden relative transform scale-100">
                <Image
                  layout="fill"
                  objectFit="cover"
                  src={`${
                    process.env.NEXT_PUBLIC_BASE_URL + product.video_thumb
                  }`}
                  alt="image"
                  className={`bgloading bg-contain bg-center ${ src !== product.product_video ? "" : "active-image" } `}
                />
              </div>
            )}
            {product.product_video_sec && product.video_thumb_sec && (
              <div
                onClick={() => changeImgHandlerVideoSecond(product.product_video_sec)}
                 className="w-[110px] h-[110px] product-video-thumb-second flex justify-center items-center overflow-hidden relative transform scale-100">
                <Image
                  layout="fill"
                  objectFit="cover"
                  src={`${
                    process.env.NEXT_PUBLIC_BASE_URL + product.video_thumb_sec
                  }`}
                  alt="image"
                  className={`bgloading bg-contain bg-center ${ src !== product.product_video_sec ? "" : "active-image" } `}
                />
              </div>
            )}
          ...
          <div className="product-details w-full mt-10 lg:mt-0">
            ...
            >
              ...
              {product.offer_price && (
                ...
                <span className="text-base font-bold text-qblack">Discount : </span>
                <span className="text-[#03a84e] font-bold">{pricePercent}%</span>
                </div>
                  ...
                  <div className="w-full">
                    <div className=" border border-qborder h-[50px] flex justify-between items-center px-6 cursor-pointer">
                      <Selectbox
						...
						defaultValue={item.active_variant_items[0].name}
                      >
                        {({ item }) => (
                          <>
			...
			{(parseInt(product.qty) !== 0) && (parseInt(product.qty) > 0)
              ...
              <div className="flex-1 h-full">
                <button
                  ...
                  type="button"
                  className="bg-qpurple hover:bg-qpurplelow hover:text-qpurple border border-transparent hover:border-qpurple transition-common text-white rounded-full text-sm font-semibold w-1/2 h-full"
                >
            ...
            {specifications &&
              specifications.length > 0 && (
                <div className="mb-[10px]">
                  <h6 className="text-[18px] font-bold mb-1">
                    Specifications:
                  </h6>
                    {specifications.map((item, i) => (
                      <p className="text-base text-qpurple leading-7" key={i}>
                      <span className="text-qblack font-bold">
                        {item.key.key} - 
                      </span>{" "}
                        {item.specification}
                      </p>
